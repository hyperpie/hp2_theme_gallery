...
if ( my_config["select_bgArt"] == "image" ){
	bgArt = fe.add_image("bg 16x9.png", 0, 0, flw, flh );
...
local bottomBar = fe.add_image("white.png",0, 680, flw + 1, 40 )
bottomBar.set_rgb( bgRed, bgGreen, bgBlue );

local bottomBarBlack = fe.add_image("white.png", 175, 682, 930, 36 )
...
class Grid extends Conveyor
{
	...
	animation.add( PropertyAnimation( frame,    {when = Transition.EndNavigation, property = "alpha", start = 0, end = 255, time = 100}));
        ...
        listB = fe.add_text("[ListEntry]/[ListSize]", 1000, 684, 300, 31);
        listB.set_rgb(0, 0, 0);
        listB.font = "gunshipbold";
        listB.align = Align.Left;
        listB.set_rgb(0, 0, 0);

        list = fe.add_text("[ListEntry]/[ListSize]", 1000, 684, 300, 30);
        ...
        ui_filter_b = fe.add_text("[FilterName] Themes", 1000, 675, 300, 20);
        ui_filter_b.align = Align.Left;
        ui_filter_b.font="gunshipbold";
        ui_filter_b.set_rgb( 0, 0, 0 );

        ui_filter_a = fe.add_text("[FilterName] Themes", 1000, 675, 300, 20);
        ...
        local statusmsg = fe.add_text("Please select a theme to download", 140, -10, 1000, 30);
        statusmsg.font="gunshipbold";	
        
        ::OBJECTS <- {
            msg = statusmsg,
            arrowL = fe.add_image("arrowL.png", 1246, 687, 25, 25),
            arrowR = fe.add_image("arrowR.png", 1246, 687, 25, 25),
        }
		...
		local move_list   = {when = Transition.StartLayout, property = "x", start = -420, end = 59, time = 800};
		local move_list2   = {when = Transition.StartLayout, property = "x", start = -420, end = 60, time = 800};	
		...
		animation.add( PropertyAnimation( OBJECTS.arrowL, {when = Transition.StartLayout, property = "y", start = 630, end = 640, time = 600, loop = true}));
...
class PopUp
{
		...
		surface.visible = false;
		local his_bg = surface.add_image( "images/his_bg.png", flx*0.250, fly*0.150, flw*0.490, flh*0.490 );
		his_bg.preserve_aspect_ratio = true;
		local his_p = surface.add_image("images/pubs/[Manufacturer]", flx*0.300, fly*0.300, flw*0.400, 0 );
		his_p.preserve_aspect_ratio = true;
		his_p.pinch_y = -90;
		his_p.alpha = 15;
		local his_w = surface.add_artwork( "wheel", flx*0.434, fly*0.170, flw*0.130, 0 );
		his_w.preserve_aspect_ratio = true;
		local history = surface.add_text("", flx*0.255, fly*0.300, flw*0.480, flh*0.320);
        ...
        {
        if ( signal == my_config["his_k"] )
            {
			animation.add( PropertyAnimation( surface, { property = "y", start = -300, end = 0, tween = Tween.Elastic, time = 3000, delay = 100 } ) );
