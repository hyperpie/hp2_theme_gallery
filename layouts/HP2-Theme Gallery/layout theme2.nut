...
class Grid extends Conveyor
{
	...
	function update_frame()
	{
		...
		name_t.index_offset = num_t.index_offset = snap_t.index_offset = wheel_t.index_offset = history_t.index_offset = get_sel() - selection_index;	
	}
...
{
fe.do_nut("star.nut");
local bg = fe.add_image( "bg2.png", 0, 0, fe.layout.width, fe.layout.height );
}
...
title.alpha = 160;
title.font = "gunshipbold";

local snapShadow= fe.add_artwork("shadow.png", 1255, 585, 757, 503 );
...
gridc.snap_t = fe.add_artwork ("flyer",  1255, 190, 580, 750 );
...
gridc.name_t.charsize = 40;
gridc.name_t.font="gunshipbold";
...
//Game List Animation
 ...
 msystem = fe.add_image( "system/[DisplayName]", 0, 0, 1920, 1080 ),
}

 local movein_mbg = {
   when =  Transition.ToNewList ,property = "alpha", start = 255, end = 255, time = 50
}

 local moveout_mbg = {
    when = Transition.ToNewList ,property = "alpha", start = 255, end = 0, time = 700, delay = 5000
}
